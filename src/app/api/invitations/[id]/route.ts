...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const inviteId = parseObjectId(resolvedParams.id);
    ...
    if (!invite) {
      return createErrorResponse("GOAL_NOT_FOUND", "We couldn't find that invitation.", 404, {
        ...
        logLevel: "info",
        context: { inviteId: resolvedParams.id, operation: "get-invitation" },
      });
    }
  ...
  try {
    ...
    if (!invite) {
      return createErrorResponse("GOAL_NOT_FOUND", "We couldn't find that invitation.", 404, {
        hint: "It may have been removed.",
        logLevel: "info",
        context: { inviteId: resolvedParams.id, operation: "update-invitation" },
      });
    }
    ...
    if (action === "decline") {
      ...
      invite.set("acceptedAt", undefined);
      await invite.save();
